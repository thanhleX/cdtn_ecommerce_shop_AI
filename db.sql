...
-- ============================================================
-- REFRESH TOKENS
-- Entity: RefreshToken (standalone, KHÔNG extends Base*)
-- Fields: id, token, user_id, expired_at, created_at
-- Indexes: idx_refresh_token (token), idx_refresh_token_user (user_id)
-- ============================================================
...
CREATE TABLE orders (
  ...
  CONSTRAINT fk_ord_payment FOREIGN KEY (payment_method_id) REFERENCES payment_methods(id) ON DELETE SET NULL,
...
CREATE TABLE blogs (
  ...
  CONSTRAINT fk_blog_user     FOREIGN KEY (user_id)          REFERENCES users(id)           ON DELETE SET NULL,
...
INSERT INTO permissions (id, name, description) VALUES
  ...
  (16, 'review:manage',   'Quản lý đánh giá')
...
INSERT INTO payment_methods (id, name, description, image) VALUES
  (1, 'Thanh toán khi nhận hàng', 'COD - Trả tiền mặt khi nhận', NULL),
  (2, 'Chuyển khoản ngân hàng',   'Chuyển khoản qua ngân hàng',  NULL),
  (3, 'Ví điện tử MoMo',          'Thanh toán qua MoMo',         NULL)
ON DUPLICATE KEY UPDATE name = VALUES(name);

-- Blog Categories mặc định
INSERT INTO blog_categories (id, name, slug, is_active) VALUES
  (1, 'Tin tức',          'tin-tuc',         TRUE),
  (2, 'Hướng dẫn',        'huong-dan',       TRUE),
  (3, 'Khuyến mãi',       'khuyen-mai',      TRUE),
  (4, 'Kiến thức',        'kien-thuc',       TRUE),
  (5, 'Thông tin khác',   'thong-tin-khac',  TRUE)
ON DUPLICATE KEY UPDATE name = VALUES(name);